...
from remotetrade.boba_zencha_probe import (
    ...
    format_boba_zencha_probe,
    format_boba_zencha_report,
    scan_boba_zencha,
...
def main() -> None:
    ...
    if args.dex_route_probe:
        ...
        if args.discord and probe.opportunity:
            maybe_send_discord(message)
        return
    if args.bsc_qash_route_probe:
        probe = scan_bsc_qash_route(
            EthereumRpcClient(settings.bsc_rpc_url),
            start_usdt=settings.bsc_qash_route_start_usdt,
            min_net_return_pct=settings.bsc_qash_route_min_net_return_pct,
        )
        append_bsc_qash_route_probe(settings.state_path.parent / "bsc_qash_route_probes.jsonl", probe)
        message = format_bsc_qash_route_probe(probe)
        print(message, flush=True)
        if args.discord and probe.opportunity:
            maybe_send_discord(message)
        return
    if args.boba_cex_dex_probe:
        probe = scan_boba_cex_dex(
            EthereumRpcClient(settings.boba_rpc_url),
            start_usd=settings.boba_cex_dex_start_usd,
            min_net_return_pct=settings.boba_cex_dex_min_net_return_pct,
    ...
    if args.boba_synapse_probe:
        probe = scan_boba_synapse(
            EthereumRpcClient(settings.boba_rpc_url),
            min_net_return_pct=settings.boba_synapse_min_net_return_pct,
            coinbase_url=settings.coinbase_url,
        )
        append_boba_synapse_probe(settings.state_path.parent / "boba_synapse_probes.jsonl", probe)
        print(format_boba_synapse_probe(probe), flush=True)
        return
    if args.boba_zencha_probe:
        path = settings.state_path.parent / "boba_zencha_probes.jsonl"
        probe = scan_boba_zencha(
            EthereumRpcClient(settings.boba_rpc_url),
            min_net_return_pct=settings.boba_zencha_min_net_return_pct,
            coinbase_url=settings.coinbase_url,
        )
        notify = should_notify_boba_zencha(path, probe)
        append_boba_zencha_probe(path, probe)
        message = format_boba_zencha_probe(probe)
        print(message, flush=True)
        if args.discord and notify:
            maybe_send_discord(message + "\n" + probe.caveat)
        return
    if args.boba_zencha_report:
        message = format_boba_zencha_report(build_boba_zencha_report(settings.state_path.parent / "boba_zencha_probes.jsonl"))
        print(message, flush=True)
        if args.discord:
            maybe_send_discord(message)
        return
