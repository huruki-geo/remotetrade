...
def scan_boba_atomic_routes(
    ...
    oolong = _read_pool(rpc, *OOLONG_USDT_USDC_POOL, block_number)
    edges = _build_edges(oolong_fee_bps)
    routes = _find_cycles(edges)
    eth_bid, _ = _coinbase_ticker(coinbase_url, "ETH-USD")
    gas_price = rpc.gas_price()
    quote_cache = {}
    all_quotes = tuple(
        _quote_route(rpc, block_number, oolong, route, size_usdc, eth_bid, gas_price, quote_cache)
...
def _build_edges(oolong_fee_bps: float) -> tuple[AtomicEdge, ...]:
    return (
        ...
        AtomicEdge("oolong", USDC, USDT, "v2", OOLONG_USDT_USDC_POOL[1], 0, 0, oolong_fee_bps),
        AtomicEdge("oolong", USDT, USDC, "v2", OOLONG_USDT_USDC_POOL[1], 0, 0, oolong_fee_bps),
    )
...
def _find_cycles(edges: tuple[AtomicEdge, ...]) -> tuple[tuple[AtomicEdge, ...], ...]:
    routes = []
    for first in edges:
        if first.token_in != USDC:
            continue
        for second in edges:
            if second.token_in == first.token_out and second.token_out == USDC and second.venue != first.venue:
                routes.append((first, second))
    return tuple(routes)


def _quote_route(rpc, block_number, oolong, route, start_usdc, eth_bid, gas_price, quote_cache) -> AtomicRouteQuote:
    amount = int(start_usdc * 10**6)
    for edge in route:
        key = (edge, amount)
        if key not in quote_cache:
            quote_cache[key] = _quote_edge(rpc, block_number, oolong, edge, amount)
    ...
    return AtomicRouteQuote(
        route=f"USDC -> USDT({route[0].venue}) -> USDC({route[1].venue})",
...
def _quote_edge(rpc, block_number, oolong, edge, amount_in):
